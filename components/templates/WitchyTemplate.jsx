...
export default function WitchyTemplate({ site }) {
  const { content, name, profession, logo, gallery, gallery_position, socials, activeSocials } = site;
  const c = content;
  ...
  // Smart services layout — no empty space
  const total = c.services.length;
  const lastRowCount = total % 3;
  const hasOrphan = lastRowCount === 1;
  ...
  function ServiceCard({ s, full }) {
    return (
      <div style={{ padding: full ? "36px 48px" : "44px 32px", border: "1px solid rgba(140,40,120,0.18)", transition: "all 0.3s ease", cursor: "default", display: full ? "flex" : "block", alignItems: full ? "center" : "flex-start", gap: full ? 40 : 0, background: "transparent" }}
        onMouseEnter={e => e.currentTarget.style.background = "rgba(120,30,100,0.12)"}
        onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
        <div style={{ fontSize: full ? 48 : 36, marginBottom: full ? 0 : 24, flexShrink: 0 }}>{s.icon}</div>
        <div>
          <h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: full ? 26 : 22, fontWeight: 400, color: "#e8c0f0", marginBottom: 12, fontStyle: "italic" }}>{s.title}</h3>
          <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 14, color: "rgba(237,224,247,0.55)", lineHeight: 1.85, fontWeight: 300 }}>{s.description}</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{ fontFamily: "system-ui,sans-serif", background: "#100818", color: "#ede0f7", minHeight: "100vh", overflowX: "hidden" }}>
      <style>{`
        ...
        @media(max-width:768px){
          ...
          .wt-hero-right{min-height:320px!important}
      ...
      {/* HERO — split layout, right shows logo/photo prominently */}
      <section className="wt-hero" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh", paddingTop: 76 }}>
        ...
        <div className="wt-hero-left" style={{ display: "flex", flexDirection: "column", justifyContent: "center", padding: "80px 56px" }}>
          <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, letterSpacing: 5, textTransform: "uppercase", color: "#8c2878", marginBottom: 28, display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ display: "inline-block", width: 24, height: 1, background: "#8c2878" }} />
            {profession}
          </p>
          <h1 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(36px,4.5vw,60px)", fontWeight: 300, lineHeight: 1.15, color: "#f0dcff", marginBottom: 28, fontStyle: "italic" }}>{c.hero.headline}</h1>
          <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 16, color: "rgba(237,224,247,0.6)", lineHeight: 1.9, marginBottom: 44, maxWidth: 420, fontWeight: 300 }}>{c.hero.subheadline}</p>
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 48 }}>
            {c.differentials.map((d, i) => (
              <div key={i} style={{ fontFamily: "'Jost',sans-serif", display: "flex", alignItems: "center", gap: 12, fontSize: 14, color: "rgba(237,224,247,0.65)", fontWeight: 300 }}>
                <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#8c2878", flexShrink: 0 }} />
                {d.text}
              </div>
            ))}
          </div>
        ...
        {/* Right — logo or first gallery photo prominently, no empty space */}
        <div className="wt-hero-right" style={{ position: "relative", overflow: "hidden", background: "linear-gradient(145deg,#1e0a2e 0%,#150520 50%,#1a0818 100%)" }}>
          ...
          {/* If they have a gallery photo, show it as hero image */}
          {gallery && gallery.length > 0 ? (
            ...
            /* If they have a logo, show it large and centered */
            ...
            /* Fallback — atmospheric design */
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", position: "relative", zIndex: 2 }}>
              <div style={{ position: "absolute", top: "15%", left: "15%", width: 220, height: 220, borderRadius: "50%", border: "1px solid rgba(140,40,120,0.15)" }} />
              <div style={{ position: "absolute", top: "22%", left: "22%", width: 130, height: 130, borderRadius: "50%", border: "1px solid rgba(140,40,120,0.12)" }} />
              <div style={{ textAlign: "center" }}>
                ...
                {!c.contact.is_online && c.contact.city && (
                  <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, letterSpacing: 5, textTransform: "uppercase", color: "rgba(140,40,120,0.45)", marginTop: 20 }}>{c.contact.city}</p>
                )}
                {c.contact.is_online && (
                  <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, letterSpacing: 4, textTransform: "uppercase", color: "rgba(140,40,120,0.45)", marginTop: 20 }}>Atendimento Online</p>
                )}
              </div>
            </div>
          )}
      ...
      <section className="wt-section wt-about" style={{ padding: "100px 56px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center", borderTop: "1px solid rgba(140,40,120,0.15)" }}>
        <div>
          ...
          <h2 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: "clamp(30px,4vw,50px)", fontWeight: 300, fontStyle: "italic", color: "#f0dcff", lineHeight: 1.2, marginBottom: 28 }}>{c.about.title}</h2>
          <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 15, color: "rgba(237,224,247,0.65)", lineHeight: 2, fontWeight: 300 }}>{c.about.text}</p>
        </div>
        <div className="wt-about-right" style={{ borderLeft: "1px solid rgba(140,40,120,0.2)", paddingLeft: 56 }}>
          ...
          <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 11, letterSpacing: 3, textTransform: "uppercase", color: "#8c2878", marginTop: 20 }}>— {c.testimonials[0]?.name}</p>
        </div>
      </section>
          ...
          <div className="wt-testi" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 2, background: "rgba(140,40,120,0.08)" }}>
            {c.testimonials.map((t, i) => (
              <div key={i} style={{ background: "#100818", padding: "36px 28px" }}>
                ...
                <p style={{ fontFamily: "'Jost',sans-serif", fontSize: 12, color: "rgba(140,40,120,0.7)", marginTop: 4 }}>{t.role}</p>
              </div>
            ))}
          </div>
        </div>
