...
const GF = `@import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@300;400;500&family=Josefin+Sans:wght@300;400');`;
  ...
  return(
    <BeamsBackground>
      ...
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center"}}>
        <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:11,letterSpacing:5,textTransform:"uppercase",color:"rgba(180,120,200,0.7)",marginBottom:32}}>✦ Powered by IA</p>
...
se inicia aqui
        ...
        <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:16,color:"rgba(200,160,220,0.6)",maxWidth:420,lineHeight:1.8,fontWeight:300,marginBottom:48}}>
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:16,marginBottom:48}}>
          <button onClick={onStart} style={{fontFamily:"'Josefin Sans',sans-serif",background:"linear-gradient(135deg,#7a1f68,#8c2878)",color:"#fff",border:"none",borderRadius:99,padding:"18px 52px",fontSize:14,letterSpacing:2,textTransform:"uppercase",cursor:"pointer",fontWeight:400}}>
            Criar meu site
          </button>
          <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:12,color:"rgba(140,40,120,0.6)",letterSpacing:1}}>R$ 197 · pagamento único · sem mensalidade</p>
        </div>
        <div style={{display:"flex",gap:40,flexWrap:"wrap",justifyContent:"center"}}>
          {["✦ Site completo em minutos","✦ Design exclusivo por IA","✦ Sem mensalidade"].map(t=>(
            <span key={t} style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:12,color:"rgba(180,120,200,0.4)",letterSpacing:1}}>{t}</span>
          ))}
        </div>
      </div>
    </BeamsBackground>
  ...
  return(
    <BeamsBackground>
      ...
      <div style={S.page}><div style={S.card}>
        <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:11,letterSpacing:4,textTransform:"uppercase",color:"rgba(140,40,120,0.7)",marginBottom:20}}>Passo 1 de 3</p>
...
function Step2({onNext,onBack,formData}){
  ...
  return(
    <BeamsBackground>
      <style>{GF}</style>
      <div style={S.page}><div style={S.card}>
        <button onClick={onBack} style={{background:"none",border:"1px solid rgba(140,40,120,0.3)",borderRadius:99,color:"rgba(200,160,220,0.5)",padding:"8px 18px",fontSize:12,cursor:"pointer",marginBottom:24,letterSpacing:1,fontFamily:"'Josefin Sans',sans-serif"}}>← Voltar</button>
        <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:11,letterSpacing:4,textTransform:"uppercase",color:"rgba(140,40,120,0.7)",marginBottom:12}}>Passo 2 de 3</p>
        ...
        {services.map((s,i)=>(
          <div key={i} style={{background:"rgba(140,40,120,0.06)",border:"1px solid rgba(140,40,120,0.2)",borderRadius:12,padding:16,marginBottom:12}}>
            <p style={{color:"rgba(140,40,120,0.7)",fontSize:10,fontWeight:500,letterSpacing:2,textTransform:"uppercase",marginBottom:10,fontFamily:"'Josefin Sans',sans-serif"}}>{serviceLabels[i].namePh}</p>
        ...
        <p style={{color:"rgba(140,40,120,0.4)",fontSize:11,marginTop:-8,marginBottom:12,fontFamily:"'Josefin Sans',sans-serif"}}>Aparece como botão abaixo dos serviços</p>
...
function Step3({onNext,onBack}){
  ...
  return(
    <BeamsBackground>
      ...
      <div style={S.page}><div style={S.card}>
        <button onClick={onBack} style={{background:"none",border:"1px solid rgba(140,40,120,0.3)",borderRadius:99,color:"rgba(200,160,220,0.5)",padding:"8px 18px",fontSize:12,cursor:"pointer",marginBottom:24,letterSpacing:1,fontFamily:"'Josefin Sans',sans-serif"}}>← Voltar</button>
        <p style={{fontFamily:"'Josefin Sans',sans-serif",fontSize:11,letterSpacing:4,textTransform:"uppercase",color:"rgba(140,40,120,0.7)",marginBottom:12}}>Passo 3 de 3</p>
...
export default function Home(){
  ...
  if(step===3)return(
    <BeamsBackground>
      <style>{GF}</style>
      <div style={{minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,fontFamily:"'Josefin Sans',sans-serif"}}>
  ...
  if(step===4&&preview){
    ...
    return(
      <div style={{minHeight:"100vh",background:"#0a0208"}}>
        <div style={{background:"rgba(10,2,8,0.95)",borderBottom:"1px solid rgba(140,40,120,0.2)",padding:"12px 20px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,position:"sticky",top:0,zIndex:200,fontFamily:"'Josefin Sans',sans-serif"}}>
        ...
        {error&&<div style={{background:"rgba(120,10,40,0.9)",color:"#fca5a5",padding:"10px 20px",fontSize:14,fontFamily:"'Josefin Sans',sans-serif"}}>⚠️ {error}</div>}
